...
def test_phase3_review_manifest_has_six_unique_contract_valid_enquiries():
    path = Path(__file__).resolve().parents[2] / "docs/fixtures/phase-3-review-manifest.json"
    manifest = json.loads(path.read_text())
    enquiries = manifest["enquiries"]
    assert len(enquiries) == 6
    assert len({item["submission_id"] for item in enquiries}) == 6
    assert len({item["correlation_id"] for item in enquiries}) == 6
    for enquiry in enquiries:
        CRMLeadCreate.model_validate(
            {
                **{key: value for key, value in enquiry.items() if key != "message"},
